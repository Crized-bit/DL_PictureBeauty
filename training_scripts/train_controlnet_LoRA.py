...
def log_validation(controlnet, args, accelerator, weight_dtype, step, is_final_validation=False
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for text_input_ids, encoder in zip(input_ids, text_encoders):

            prompt_embeds = encoder(text_input_ids, output_hidden_states=True)
        ...
        if pipeline.config.force_zeros_for_empty_prompt:
...
def make_train_dataset(args, tokenizer, tokenizer_2, accelerator):
    ...
    def preprocess_train(examples):
        ...
        examples["input_ids_2"] = tokenize_captions(examples, tokenizer=tokenizer_2)
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = ControlNetModel.from_pretrained(args.controlnet_model_name_or_path)
                load_model = PeftModel.from_pretrained(model=load_model, 
                                                       model_id=input_dir + "/controlnet", 
                                                       is_trainable=True)
                model.register_to_config(**load_model.config)

                set_peft_model_state_dict(model, get_peft_model_state_dict(load_model))
    ...
    r=args.lora_rank,#256,
    lora_alpha=args.lora_alpha,#256,
    ...
    controlnet = get_peft_model(controlnet, peft_config=lora_config)
    ...
    # Optimizer creation
    lora_layers = filter(lambda p: p.requires_grad, controlnet.parameters())
    ...
    train_dataset = make_train_dataset(args, tokenizer, tokenizer_2, accelerator)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                prompt_embeds_list = []
                input_ids = [batch["input_ids"], batch["input_ids_2"]]
                ...
                # print(pooled_prompt_embeds.shape[0])
    ...
    if accelerator.is_main_process:
        ...
        controlnet.save_pretrained(args.output_dir + "/" + args.tracker_project_name)
